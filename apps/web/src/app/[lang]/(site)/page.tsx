...
function destinationsFor(
  ...
  if (roles.includes('student') || roles.includes('adult_learner')) {
    // §4: one destination, because the learner surface has its own navigation.
    // Two links into the middle of it from here would be a second, competing
    // answer to "where do I go", and UI-005's five-item budget is spent inside
    // the dashboard rather than in front of it.
    items.push({ href: `/${language}/student`, label: t('student.home.title'), icon: '🎒' });
  }
